...
def main_process(limit,start=0,end=1000000,minDP=0,deNovo=False):
    if limit>=10:
    ...
    proc=VCF_sample_processor(start,end,minDP,deNovo )
...
if __name__ == '__main__':
    if len(sys.argv)==2:
    ...
    elif  not (2<=len(sys.argv)<=6):
        print("Usage: python3 main2.py <limit> <start> <end> <minDP> <deNovo>")
        ...
        if len(sys.argv)==3:
            ...
            minDP = float(sys.argv[4])
            deNovo= bool(sys.argv[5])
            main_process(limit,start, end, minDP,deNovo)
